...
class SmvContext(object):
    """
    Initialize a SMV context where a SparkContext and a HiveContext are maintained
    and SMV apps can be created and submitted.
    """
    def __init__(self, config):
        self.sc = SparkContext(appName=config['name'])
        self.sc.setLogLevel(config['log_level'])
        self.sqlContext = HiveContext(self.sc)
        self.import_java_packages()
    ...
    def create_smv_app(self, arglist):
        ...
        self._jsmv = self.sc._jvm.org.tresamigos.smv.python.SmvPythonAppFactory.init(java_args, self.sqlContext._ssql_ctx)
        return self._jsmv

    def import_java_packages(self):
        java_import(self.sc._jvm, "org.tresamigos.smv.ColumnHelper")
        java_import(self.sc._jvm, "org.tresamigos.smv.SmvDFHelper")
        java_import(self.sc._jvm, "org.tresamigos.smv.dqm.*")
        java_import(self.sc._jvm, "org.tresamigos.smv.python.SmvPythonHelper")

    def start_callback_server(self):
        gw = self.sc._gateway
        cbs_port = gw._python_proxy_port #TODO: should allow user to change port
...
class SmvPyDataSet(object):
    ...
    def smvDQM(self):
        ...
        return self.smvapp.sc._jvm.SmvDQM.apply()

    # Factory methods for DQM policies
    def FailParserCountPolicy(self, threshold):
        return self.smvapp.sc._jvm.FailParserCountPolicy(threshold)

    def FailTotalRuleCountPolicy(self, threshold):
        return self.smvapp.sc._jvm.FailTotalRuleCountPolicy(threshold)

    def FailTotalFixCountPolicy(self, threshold):
        return self.smvapp.sc._jvm.FailTotalFixCountPolicy(threshold)

    def FailTotalRulePercentPolicy(self, threshold):
        return self.smvapp.sc._jvm.FailTotalRulePercentPolicy(threshold * 1.0)

    def FailTotalFixPercentPolicy(self, threshold):
        return self.smvapp.sc._jvm.FailTotalFixPercentPolicy(threshold * 1.0)

    # DQM task policies
    def FailNone(self):
        return self.smvapp.sc._jvm.DqmTaskPolicies.failNone()

    def FailAny(self):
        return self.smvapp.sc._jvm.DqmTaskPolicies.failAny()

    def FailCount(self, threshold):
        return self.smvapp.sc._jvm.FailCount(threshold)

    def FailPercent(self, threshold):
        return self.smvapp.sc._jvm.FailPercent(threshold * 1.0)

    def DQMRule(self, rule, name = None, taskPolicy = None):
        task = taskPolicy or self.FailNone()
        return self.smvapp.sc._jvm.DQMRule(rule._jc, name, task)

    def DQMFix(self, condition, fix, name = None, taskPolicy = None):
        task = taskPolicy or self.FailNone()
        return self.smvapp.sc._jvm.DQMFix(condition._jc, fix._jc, name, task)
...
class SmvPyCsvFile(SmvPyDataSet):
    ...
    def __mkCsvAttr(self, delimiter=',', quotechar='""', hasHeader=False):
        ...
        return self.smvapp.sc._jvm.org.tresamigos.smv.CsvAttributes(delimiter, quotechar, hasHeader)
...
class SmvPyCsvStringData(SmvPyDataSet):
    ...
    def __init__(self, smvapp):
        ...
        self._smvCsvStringData = self.smvapp.sc._jvm.org.tresamigos.smv.SmvCsvStringData(
            self.schemaStr(),
            self.dataStr(),
            False
        )
...
class SmvPyHiveTable(SmvPyDataSet):
    ...
    def __init__(self, smvapp):
        ...
        self._smvHiveTable = self.smvapp.sc._jvm.org.tresamigos.smv.SmvHiveTable(self.tableName())
