...
take some actions on it. However, we could owner the flag by not persisting the data.
```scala
def computeRDD() {
  if(isEphemeral) {
    val resultDf = doRun().attachDQM(dqm)
    resultDf.checkDQMWithAction(dqm)
    resultDf
  } else {
    readPersistedFile() recoverWith {case e =>
      persist(doRun)
      val rddReadBack = readPersistedFile(prefix).get
      rddReadBack.checkDQMWithoutAction(dqm)
      rddReadBack
    }
  }
}
