...
class SmvRunInfoCollectorSpec extends SmvUnitSpec {
  ...
  it should "keep all datasets for which there is a validation result" in {
    val target = new SmvRunInfoCollector
    target.addRunInfo("a", new DqmValidationResult(true, null), null)
    target.addRunInfo("b", new DqmValidationResult(false, null), null)
    target.addRunInfo("c", new DqmValidationResult(false, null), null)
  ...
  it should "not accept null for dataset fqn" in {
    ...
    intercept[IllegalArgumentException] {
      target.addRunInfo(null, new DqmValidationResult(true, null), null)
    }
  }
  ...
  it should "not accept empty string for dataset fqn" in {
    val target = new SmvRunInfoCollector
    intercept[IllegalArgumentException] {
      target.addRunInfo("", new DqmValidationResult(true, null), null)
    }
  }
}
