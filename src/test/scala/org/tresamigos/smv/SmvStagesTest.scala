...
package org.tresamigos.smv {
  ...
  class SmvWhatModulesToRunTest extends SparkTestUtil {
    test("Test modules to run (non-output module)") {
      object testApp
          extends SmvApp(
            testAppArgs.multiStage ++ Seq("-m", "org.tresamigos.smv.smvAppTestPkg3.T"),
            Some(sparkSession)) {}
    ...
    test("Test modules to run (mods in stage)") {
      object testApp
          extends SmvApp(testAppArgs.multiStage ++ Seq("-s", "smvAppTestPkg1"), Some(sparkSession)) {}
      val mods = testApp.modulesToRun.map(_.fqn)
      println(mods)
    ...
    test("Test modules to run (mods in app)") {
      object testApp
          extends SmvApp(testAppArgs.multiStage ++ Seq("--run-app"), Some(sparkSession)) {}
      val mods = testApp.modulesToRun.map(_.fqn)
      println(mods)
