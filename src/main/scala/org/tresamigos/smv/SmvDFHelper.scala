...
class SmvDFHelper(df: DataFrame) {
  ...
  /**
   * Carries out the following transformation
   *
   *    +-----+-----+            +-----+------+------+
   *    | k1  | k2  |            | k2  | k1_1 | k1_2 |
   *    +-----+-----+            +-----+------+------+
   ...
   *
   *     +-----+-----+------+            +-----+------+------+------+------+
   *     | k1  | k2  | rank |            | k2  | k1_1 | k1_2 | k1_3 | k1_4 |
   *     +-----+-----+------+            +-----+------+------+------+------+
   *     |  A  |  X  |  1   |            |  X  |  A   | null | null | null |
   *     +-----+-----+------+    ===>    +-----+------+------+------+------+
   *     |  B  |  Y  |  2   |            |  Y  | null |  B   | null |  C   |
   *     +-----+-----+------+            +-----+------+------+------+------+
   *     |  C  |  Y  |  4   |
   *     +-----+-----+------+
   *
   * The transformation reduces pair data (between k1 and k2) from row
   * representation (where each k2 may have multiple rows, one for
   * each relationship to k1), to column representation (where each k2
   * has only one row, in which there is one non-null column for each
   * k1 relationship).
   *
   * The caller may optionally supply a column containing the rank of
   * k1 within each group of k2.  If the column does not already exist
   * in the dataframe, then an intermediate 'rank' column will be
  ...
  def smvPivotCoalesce(pivotCol: String)(
    ...
    // collect the distinct values of the ranking
    val range = ranked.select(rcol).distinct.collect.map(_.get(0))

    // pivot transformation is n-in-n-out
    val pivoted = ranked.smvPivot(Seq(rcol))(valueCol)(range.map(_.toString):_*)

    // now we just need to collapse each group into 1 row
    // construct the column names for transformed row
    val cols = for {
      r <- range
      n = s"${valueCol}_${r}"
    } yield smvFirst($"$n", true) as n

    pivoted.groupBy(pivotCol).agg(cols.head, cols.tail:_*)
  }
