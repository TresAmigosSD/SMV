...
private[smv] object smvStddev extends UserDefinedAggregateFunction {
  ...
  def bufferSchema =
    new StructType().add("count", LongType).add("avg", DoubleType).add("m2", DoubleType)
...
private[smv] case class SmvFirst(child: Expression)
    extends UnaryExpression
    with AggregateExpression1 {
  def this() = this(null)
...
private[smv] case class SmvFirstFunction(expr: Expression, base: AggregateExpression1)
...
private[smv] object DoubleBinHistogram extends UserDefinedAggregateFunction {
  ...
  val return_type_struct_fields = Array(StructField("interval_low_bound", DoubleType),
   ...
   */
  private def bin_interval(bin_index: Int,
                           min_val: Double,
                           max_val: Double,
                           num_of_bins: Int): (Double, Double) = {
    val interval_length = (max_val - min_val) / num_of_bins
